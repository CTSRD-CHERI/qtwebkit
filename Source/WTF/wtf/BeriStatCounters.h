...
static inline bool statcountersShouldPrint()
{
    static const char* verbose = getenv("VERBOSE");
...
static inline void statcountersEndPhase(const statcounters_bank_t *stat_start, const std::string& phase) {
    ...
    if (statcountersShouldPrint())
        fprintf(stderr, " -- Finished phase %s after %jd instructions(%jd user/%jd kernel)\n",
           phase.c_str(), (intmax_t)stat_diff.inst, (intmax_t)stat_diff.mipsmem[2], (intmax_t)stat_diff.mipsmem[4]);
}
